package dev.overgrown.sync.power.type.summons.entities.clone;
...
/**
 * Clones are "tamed" entities that visually mirror the player who summoned them.
 * They are hostile-typed so vanilla BowAttackGoal works without a bespoke version.
 */
public class CloneEntity extends HostileEntity implements ExtraTameable, CrossbowUser, Temporary {
    ...
    protected void initDataTracker(DataTracker.Builder builder) {
        ...
        builder.add(CUSTOM_WIDE_TEXTURE_NAMESPACE, "");
        builder.add(CUSTOM_WIDE_TEXTURE_PATH, "");
        builder.add(CUSTOM_SLIM_TEXTURE_NAMESPACE, "");
        builder.add(CUSTOM_SLIM_TEXTURE_PATH, "");
    }
    ...
    public void tick() {
        super.tick();
        if (this.getOwner() == null || this.getOwner().getWorld().getRegistryKey() != this.getWorld().getRegistryKey()
            || (this.age > this.maxTicks && this.maxTicks > 0)) {
            this.discard();
            ...
            for (int i = 0; i < 20; ++i) {
                ...
                this.getWorld().addParticle(ParticleTypes.POOF, this.getParticleX(1.0), this.getRandomBodyY(),
                    this.getParticleZ(1.0), velX, velY, velZ);
            }
        }
    }
    ...
    public void shootAt(LivingEntity target, float pullProgress) {
        ...
        Hand bowHand = ProjectileUtil.getHandPossiblyHolding(this, Items.BOW);
        ItemStack bowStack = this.getStackInHand(bowHand);
        ItemStack arrowType = this.getProjectileType(bowStack);
        PersistentProjectileEntity persistentProjectileEntity =
            ProjectileUtil.createArrowProjectile(this, arrowType, pullProgress, bowStack);
        ...
        persistentProjectileEntity.setVelocity(xDirection, yDirection + g * 0.2, zDirection,
            1.6f, 14 - this.getWorld().getDifficulty().getId() * 4);

        this.getWorld().playSound(null, this.getX(), this.getY(), this.getZ(),
            SoundEvents.ENTITY_ARROW_SHOOT, SoundCategory.PLAYERS, 1.0f,
            1.0f / (this.getWorld().getRandom().nextFloat() * 0.4f + 1.2f) + pullProgress * 0.5f);
    ...
    public boolean canTarget(LivingEntity target) {
        if (this.getOwner() == null || !target.canTakeDamage() || target == this.getOwner() || this.isSitting() || !this.canAttack
            || (this.getScoreboardTeam() != null && target.getScoreboardTeam() != null
                && this.getScoreboardTeam().isEqual(target.getScoreboardTeam())))
            return false;
    ...
    protected static class DefendOwnerGoal extends AssistOwnerGoal {
        ...
        @Override
        public boolean canStart() {
            if (super.canStart() && this.owner.getAttacker() != null
                && this.clone.canTarget(this.owner.getAttacker())
    ...
    protected static class FightForOwnerGoal extends AssistOwnerGoal {
        ...
        @Override
        public boolean canStart() {
            if (super.canStart() && this.owner.getAttacking() != null
                && this.clone.canTarget(this.owner.getAttacking())
