package dev.overgrown.sync.factory.power.type.edible_item;
...
public class EdibleItemPower extends Power implements Prioritized<EdibleItemPower> {
    ...
    public EdibleItemPower(PowerType<?> powerType, LivingEntity livingEntity, Consumer<Entity> entityAction, Consumer<Pair<World, ItemStack>> consumedItemAction, Consumer<Pair<World, ItemStack>> resultItemAction, Predicate<ItemStack> itemCondition, FoodComponent foodComponent, ItemStack resultStack, ConsumeAnimation consumeAnimation, SoundEvent consumeSoundEvent, Modifier consumingTimeModifier, List<Modifier> consumingTimeModifiers, int priority) {
        ...
        if (consumingTimeModifier != null) this.consumingTimeModifiers.add(consumingTimeModifier);
        if (consumingTimeModifiers != null) this.consumingTimeModifiers.addAll(consumingTimeModifiers);
    ...
    public ItemStack executeItemActions(ItemStack consumedStack) {
        ...
        ItemStack result = this.resultStack != null ? this.resultStack.copy() : ItemStack.EMPTY;
        if (!result.isEmpty() && resultItemAction != null) {
            resultItemAction.accept(new Pair<>(entity.getWorld(), result));
        }
        return result;
    }
